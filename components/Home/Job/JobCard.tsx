...
const JobCard = ({job}: Props) => {
  return (
    <div className='border-[1.5px] border-gray-300 dark:border-gray-700 p-6 rounded-lg relative'>
        {/* Bookmark icon */}
      <div className='w-7 h-7 cursor-pointer hover:bg-gray-300 dar:hover:bg-gray-800 flex items-center transition-all duration-200 justify-center rounded-full flex-col absolute top-4 right-4'>
      ...
      <div className='flex items-center space-x-4 mt-3'>
        ...
        <div className={`px-4 py-1 rounded-full text-xs ${job.urgency === 'Urgent' ? "bg-red-600/30 dark:bg-red-300/30 text-red-700 dark:text-red-300":"bg-green-600/30 text-green-700 dark:text-green-300"}`}>
            {job.urgency}
        </div>
      </div>
    </div>
